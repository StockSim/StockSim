...
//    private static final String BASE_URL = "https://www.alphavantage.co/query?function=REALTIME_BULK_QUOTES";
        ...
        try {
            ...
            try (Response response = client.newCall(request).execute()) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    for (Object quote : jsonResponse.getJSONArray("Global Quote")) {
    ...
    public static void main(String[] args) {
        ExternalStockDataAccessObject externalStockDataAccessObject = new ExternalStockDataAccessObject();
        List<String> tickers = new ArrayList<>();
        tickers.add("APPL");
        System.out.println(externalStockDataAccessObject.getStocks(tickers));
    }
}